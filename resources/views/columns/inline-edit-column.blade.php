...
@endphp
    ...
    <div 
        ...
        // Save state function
        saveState() {
            if (this.saving) return; // Prevent double-save
            this.saving = true;
            ...
            $wire.updateTableColumnState(@js($name), @js($recordKey), this.state)
                .then(() => {
                ...
                .catch((error) => {
                    ...
                    this.errors = error.response?.data?.errors || error.message || ['Update failed'];
                    this.saving = false;
                    ...
                    this.clearErrorTimeout();
                    this.timeoutId = setTimeout(() => { this.errors = null; this.timeoutId = null; }, 5000);
                });
        },
...
>
    ...
    >
        ...
        @if($isDateField)
            <svg class="inline-block w-3 h-3 ml-1 opacity-30 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div x-show="isEditing" class="flex items-center space-x-1" x-cloak>
        @if($isDateField)
            ...
            <div class="relative">
                <input 
                    ...
                    class="border border-gray-300 dark:border-gray-600 rounded px-2 py-1 text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 min-w-[180px]"
                    @keydown.enter="saveState()"
                    @keydown.escape="cancelEdit()"
                    @blur="if (!saving) { cancelEdit(); }"
                    :disabled="saving"
                />
        ...
        @elseif($type === 'select')
            <!-- Select Dropdown (for future enhancement) -->
            <select 
                x-ref="input"
                x-model="state"
...
<style>
    ...
    .inline-edit-column input[type="datetime-local"],
    .inline-edit-column input[type="date"] {
        min-width: 180px;
    }
